...
### Calibration (separate from `test:e2e`)

E2E step timeouts come from `.calibration.json`, generated by running all `@calibrate`-tagged scenarios. This is **not** part of `pnpm test:e2e` — run it explicitly when the file is missing or after changing heavy scenarios:
...
CI runs `test:e2e:calibrate` once per shard job before `test:e2e`. Sharding (`TIDGI_E2E_SHARD`) applies only to the scenario run, not to calibration.
