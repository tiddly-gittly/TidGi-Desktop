...
After `pnpm install`, if `@memeloop/prompt-editor` is linked from the local memeloop monorepo, run once:

```bash
pnpm run build:memeloop-prompt-editor
```

so `dist/` exists for TypeScript types and any tooling that resolves the package entry.
...
6. **Don't think about adding new step definitions** or **change timeout duration**, unless human ask you to. You should always reuse existing steps, and debug the fundamental reason that causes timeout. Timeout usually because of expected element not percent.
