...
export interface IGitService {
  clone(remoteUrl: string, repoFolderPath: string, userInfo: IGitUserInfos): Promise<boolean>;
  /**
   * Commit unstaged or staged changes and sync to cloud
   * @returns hasChange boolean, means whether we have new git changes
   */
  commitAndSync(wikiFolderPath: string, remoteUrl: string, userInfo: IGitUserInfos): Promise<boolean>;
  /**
   ...
   */
  debounceCommitAndSync: (wikiFolderPath: string, remoteUrl: string, userInfo: IGitUserInfos) => Promise<boolean> | undefined;
  ...
  initWikiGit(wikiFolderPath: string, isSyncedWiki?: false): Promise<boolean>;
  /**
   * Run git init in a folder, prepare remote origin if isSyncedWiki
   */
  initWikiGit(wikiFolderPath: string, isSyncedWiki: true, remoteUrl: string, userInfo: IGitUserInfos): Promise<boolean>;
