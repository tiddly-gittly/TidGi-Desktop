...
type WindowsExternalEditor = {
   ...
   */
  readonly registryKeys: readonly RegistryKey[];
...
const wow64UninstallSubKey = 'SOFTWARE\\WOW6432Node\\Microsoft\\Windows\\CurrentVersion\\Uninstall';

const CurrentUserUninstallKey = (subKey: string) => registryKey(HKEY.HKEY_CURRENT_USER, uninstallSubKey, subKey);

const LocalMachineUninstallKey = (subKey: string) => registryKey(HKEY.HKEY_LOCAL_MACHINE, uninstallSubKey, subKey);

const Wow64LocalMachineUninstallKey = (subKey: string) => registryKey(HKEY.HKEY_LOCAL_MACHINE, wow64UninstallSubKey, subKey);
...
const registryKeysForJetBrainsIDE = (product: string): readonly RegistryKey[] => {
...
const executableShimPathsForJetBrainsIDE = (baseName: string): ReadonlyArray<readonly string[]> => {
  return [
    ['bin', `${baseName}64.exe`],
    ['bin', `${baseName}.exe`],
  ];
...
function getAppInfo(editor: WindowsExternalEditor, keys: readonly RegistryValue[]): IWindowsAppInformation {
  ...
  const installLocation = getKeyOrEmpty(keys, editor.installLocationRegistryKey ?? 'InstallLocation');
