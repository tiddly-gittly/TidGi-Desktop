...
async function executeGitLog(parameters: GitLogParameters): Promise<ToolExecutionResult> {
  ...
  const result = await gitService.callGitOpForWorkspace(workspaceID, 'getGitLog', wikiFolderLocation, options);
...
async function executeGitReadFile(parameters: GitReadFileParameters): Promise<ToolExecutionResult> {
  ...
  const fileResult = await gitService.callGitOpForWorkspace(
    workspaceID,
    'getFileContent',
    wikiFolderLocation,
