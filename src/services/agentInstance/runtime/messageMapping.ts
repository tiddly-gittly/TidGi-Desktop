...
export function toConversationMeta(agent: AgentInstance, originNodeId: string, _definition?: AgentDefinition): ConversationMeta {
  const messages = agent.messages ?? [];
  const last = messages[messages.length - 1];
  const ts = last?.timestamp ?? Date.now();
  const originClock = messages
    .filter(message => message.originNodeId === originNodeId)
    .reduce((max, message) => Math.max(max, message.lamportClock), 1);
  return {
