    ...
    if (addresses !== undefined) {
      for (const { cidr } of addresses) {
        if (cidr) {
          ...
          if (
            net[0] && net[0].kind() === gatewayIp.kind() &&
            /**
             * TypeScript can't narrow IPv4/IPv6 union overloads on .match(), but the kind()
             * check above guarantees runtime compatibility — both sides share the same address family.
             */
            // eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access
            (gatewayIp as any).match(net)
          ) {
