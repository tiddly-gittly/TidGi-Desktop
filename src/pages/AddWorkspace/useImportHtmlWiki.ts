...
export function useImportHtmlWiki(
    ...
    const onSubmit = useCallback(async () => {
        ...
        try {
            ...
            var extractState = await window.service.wiki.extractWikiHTML(form.wikiHtmlPath, form.extractWikiHtmlParentFolder);
            // var extractState用于接收执行是否解压是否成功。
            // 我希望在解压成功后执行打开解压后的wiki文件夹的操作。并且设置好工作区的信息。
    ...
    }, [wikiCreationMessageSetter, t, hasErrorSetter, form, isCreateMainWorkspace, isCreateSyncedWorkspace, form.wikiHtmlPath, form.extractWikiHtmlParentFolder, errorInWhichComponentSetter]);
