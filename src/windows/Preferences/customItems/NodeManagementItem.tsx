      ...
      {status === null
        ...
        ? (
          <Alert severity='info' sx={{ mb: 1 }} icon={<SyncIcon />}>
            {t('Preference.WikiSync.NodeRunning', { ns: 'translation', port: status.port, nodeId: status.nodeId?.slice(0, 16) })}
          </Alert>
        )
        : (
          <Alert severity='warning' sx={{ mb: 1 }}>
            {t('Preference.WikiSync.NodeNotRunning', { ns: 'translation' })}
          </Alert>
        )}
  ...
  return (
    <Box sx={{ p: 1.5, mb: 1 }}>
      <Typography variant='subtitle2' sx={{ mb: 1 }}>
        ...
        {t('Preference.NodeIdentity')}
      </Typography>
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
        <Typography variant='body2'>
          {t('Preference.WikiSync.NodeId', { ns: 'translation' })}: <code>{identity.nodeId.slice(0, 24)}…</code>
        </Typography>
        {pinCode && (
          <Typography variant='body2'>
            {t('Preference.WikiSync.PinCode', { ns: 'translation' })}: <strong>{pinCode}</strong>
          </Typography>
        )}
        <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mt: 0.5 }}>
          {identity.hasKeypair
            ? <Chip icon={<VpnKeyIcon />} label={t('Preference.WikiSync.KeypairReady', { ns: 'translation' })} color='success' size='small' variant='outlined' />
            : <Chip label={t('Preference.WikiSync.NoKeypair', { ns: 'translation' })} color='warning' size='small' variant='outlined' />}
          <Chip label={t('Preference.WikiSync.KnownNodeCount', { ns: 'translation', count: identity.knownNodeCount })} size='small' variant='outlined' />
        </Box>
      </Box>
    </Box>
  );
      ...
      <List dense disablePadding>
        {nodes.map((node) => (
          <Box key={node.nodeId}>
            <ListItem
              secondaryAction={
                <Tooltip title={t('Preference.WikiSync.RemoveTrust', { ns: 'translation' })}>
  ...
  return (
    <Box sx={{ p: 1.5, mb: 1 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box>
          <Typography variant='body2'>
            {t('Preference.SyncStatus')}: {t('Preference.WikiSync.PeerCount', { count: syncStatus.peerCount, ns: 'translation' })}
            {syncStatus.syncRunning && <Chip label={t('Preference.WikiSync.SyncActive', { ns: 'translation' })} color='success' size='small' variant='outlined' sx={{ ml: 1 }} />}
          </Typography>
        </Box>
        <Button
          size='small'
          ...
          disabled={syncing}
        >
          {t('Preference.WikiSync.SyncNowButton', { ns: 'translation' })}
        </Button>
      </Box>
    </Box>
  );
    ...
    <Box sx={{ p: 1.5, mb: 1 }}>
      ...
      <Box sx={{ display: 'flex', gap: 1, alignItems: 'flex-start' }}>
        ...
        <Button
          ...
          sx={{ minWidth: 80 }}
        >
          {connecting ? <CircularProgress size={16} /> : t('Preference.WikiSync.Connect', { ns: 'translation' })}
        </Button>
      </Box>
    </Box>
  );
    ...
    <>
      <Box sx={{ display: 'flex', alignItems: 'center', mt: 1, mb: 0.5 }}>
        <Typography variant='subtitle2'>
          ...
          {t('Preference.WikiSync.RemoteNodes', { ns: 'translation' })}
          {peers.length > 0 && ` (${peers.length})`}
        </Typography>
        <Tooltip title={t('Preference.WikiSync.Refresh', { ns: 'translation' })}>
