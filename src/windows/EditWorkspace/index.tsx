...
import {
  RestartSnackbarType,
  useRestartSnackbar,
} from "@/components/RestartSnackbar";
...
export default function EditWorkspace(): React.JSX.Element {
  const [workspaceID, setWorkspaceID] = useState<string | undefined>(
    () => (window.meta() as WindowMeta[WindowNames.editWorkspace]).workspaceID,
  );
  ...
  useEffect(() => {
    if (
      ...
      return;
    void window.service.workspace
      .get(workspaceID)
      .then((workspaceFromService) => {
  ...
  const [requestRestartCountDown, RestartSnackbar] = useRestartSnackbar({
    waitBeforeCountDown: 0,
    workspace: originalWorkspace,
    restartType: RestartSnackbarType.Wiki,
  });
  const [workspace, workspaceSetter, onSave] = useForm(
    currentWorkspace,
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      const id = (window.meta() as WindowMeta[WindowNames.editWorkspace])
        .workspaceID;
      if (id) {
        setWorkspaceID(id);
        clearInterval(timer);
  ...
  return (
    <Outter>
      ...
      <Helmet>
        <title>
          {t("WorkspaceSelector.EditWorkspace")} {String(workspace.order ?? 1)}{" "}
          {workspace.name}
        </title>
      </Helmet>
      ...
      {wikiWorkspace && (
        <WorkspaceFormProvider
          ...
          workspaceSetter={
            workspaceSetter as (ws: typeof wikiWorkspace, nr?: boolean) => void
          }
        >
          {isSearching ? (
            <Inner>
      ...
      {!isEqual(
        omit(workspace, nonConfigFields),
        omit(originalWorkspace, nonConfigFields),
      ) && (
        <SaveCancelButtonsContainer>
          ...
          <Button
            variant="contained"
            disableElevation
            onClick={() => void window.remote.closeCurrentWindow()}
