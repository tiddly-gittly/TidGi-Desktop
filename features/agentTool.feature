Feature: Agent Tools - Ask-question variants and turn action bar
  ...
  Scenario: Agent switcher — switch between Task Agent and Plan Agent
    ...
    And I should see a "plan agent option" element with selector "[data-testid='agent-switcher-listbox'] [role='option']:has-text('代码助手')"
    When I click on a "plan agent option" element with selector "[data-testid='agent-switcher-listbox'] [role='option']:has-text('代码助手')"
