...
describe('validateCucumberArguments', () => {
  ...
  it('allows --tags=@smoke (combined form)', () => {
    expect(() => {
      validateCucumberArguments(['--tags=@smoke']);
    }).not.toThrow();
  });
  ...
  it('blocks bare positional tag values', () => {
    expect(() => {
      validateCucumberArguments(['@html-wiki']);
    }).toThrow(E2EArgsValidationError);
  });

  it('allows --name filter with spaced title', () => {
    expect(() => {
      validateCucumberArguments(['--name', 'Wiki-search tool usage']);
    }).not.toThrow();
  });
  ...
  it('blocks combining --tags and --name', () => {
    expect(() => {
      validateCucumberArguments(['--tags=@html-wiki', '--name', 'HTML workspace git log only shows the managed html file']);
    })
      .toThrow(E2EArgsValidationError);
  });
  ...
  it('blocks unknown options', () => {
    expect(() => {
      validateCucumberArguments(['--foo=bar']);
    }).toThrow(E2EArgsValidationError);
  });

  it('blocks --name without a value', () => {
    expect(() => {
      validateCucumberArguments(['--name']);
    }).toThrow(E2EArgsValidationError);
  });
});
